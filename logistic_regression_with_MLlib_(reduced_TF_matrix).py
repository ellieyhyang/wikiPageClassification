...
Description: This script contains a pySpark program that used spark ml
             library to train a logistic regression model the given 
             data(Wiki pages) based on a subset of features that are 
             selected based on variance from the term frequency of the 
             top 20k words of the corpus.  
"""
...
if __name__ == "__main__":
    ...
    # - STEP 3: Select a subset of features based on their variance
    ...
    docLabelAndTFArray = allDocsAsNumpyArrays_Reduced\
    ...
    # - STEP 2: Select the same 10k features as the training set

    test_allDocsAsNumpyArrays_Reduced = test_allDocsAsNumpyArrays\
                                    .map(lambda x: (x[0], x[1][top10kVarWordIdx]))


    # - STEP 3: Label the testing data

    # Create an RDD that has (label, TFArray) pairs for all documents
    # labeling: 1 if docID starts with 'AU' (Australia court case) and 0 if others
    # Then map it to RDD of (label, Vector.dense) pairs
    test_docLabelAndTFArray = test_allDocsAsNumpyArrays_Reduced\
                                .map(lambda x: (1, x[1]) if x[0][:2] == 'AU' else (0, x[1]))\
                                .map(lambda x: (x[0], Vectors.dense(x[1])))
    ...
    # Train the model
    lr_model = LogisticRegression(maxIter=100).fit(labelTFVectorDF)

    # log the end time
    end_lr_train = time.time()
    print("\nTime(sec) taken to train the logistic regression model:", end_lr_train - start_lr_train)

    
    '''------------------- Model Testing ------------------- '''

    # Log the start time
    start_lr_test = time.time()

    # Make predictions for the test sets
    # Resulted RDD contains (pred, label) pairs
    lr_predAndLabel =  lr_model.transform(test_labelTFVectorDF)\
    ...
    lr_metrics = MulticlassMetrics(lr_predAndLabel)
    ...
    print("F-score", lr_metrics.fMeasure(1.0))

    # Log the end time
    end_lr_test = time.time()
    print("\nTime(sec) taken to test the model:", end_lr_test - start_lr_test)
